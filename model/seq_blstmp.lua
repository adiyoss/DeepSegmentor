------------------------------------------------------------------------
--[[ SeqBRNN ]] --
-- Bi-directional RNN using two SeqLSTMP modules.
...
------------------------------------------------------------------------
local SeqBRNN, parent = torch.class('nn.SeqBLSTMP', 'nn.Container')

function SeqBRNN:__init(inputDim, hiddenDim, batchFirst, merge)
    self.forwardModule = nn.SeqLSTMP(inputDim, hiddenDim, hiddenDim)
    self.backwardModule = nn.SeqLSTMP(inputDim, hiddenDim, hiddenDim)
    self.merge = merge
