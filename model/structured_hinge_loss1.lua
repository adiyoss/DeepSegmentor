...
function StructuredHingeLoss:predict(input, k)  
  ...
  local end_search = (start_search + input:size(1) / k)
  local max_val = 0
  local onsets_hat = {}
  local output = 0
  for i=1, k do
    local max_k = 0
    local best_onset = 1    
    for j=start_search, math.min(start_search + (input:size(1) / k), input:size(1)) do
      local tmp = input[j]
      if tmp > max_k then
        max_k = tmp  
        best_onset = j
    ...
    end_search = (i == k) and math.min(start_search + (input:size(1) / k), input:size(1)) or input:size(1)
  end 
