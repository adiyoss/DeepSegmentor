...
def extract_single_acoustic(in_path, out_path):
    ...
    length = utils.get_wav_file_length(tmp_file)

    with open(input_file, 'w') as f:
        # write the data
        f.write(
            '"' + tmp_file + '" ' + str('%.8f' % 0) + ' ' + str(float(length) - zero) + ' ' + str(
                '%.8f' % 0) + ' ' + str('%.8f' % 0))
...
def create_labels(in_path, out_path):
    ...
    for item in os.listdir(in_path):
        if item.endswith(".TextGrid"):
            ...
            onset = np.ceil(textgrid.tiers[0].intervals[1].minTime * 1000)
            offset = np.ceil(textgrid.tiers[0].intervals[1].maxTime * 1000)
...
def main(in_path_x, in_path_y, out_path):
    ...
    extract_acoustic_dir(in_path_x, out_path)  # extract the features
